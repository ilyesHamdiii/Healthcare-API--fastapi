...
@router.patch("/update/{id}",status_code=status.HTTP_200_OK,response_model=schemas.ResArticle)
def update_article(id: int,article: schemas.CreateArticle,db: Session = Depends(get_db),current_user: models.User = Depends(oauth.get_current_user)):
...
def delete(id:int,db:Session=Depends(get_db),current_user: models.User = Depends(oauth.get_current_user)):
    ...
    if article.id!=current_user.id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail=f"You are not authorised to delete this post")
    db.delete(article)
    db.commit()
